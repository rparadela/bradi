...
# Script 07 — Calculate BrADI (2022)
# =============================================================================
# Description:
#   Loads standardized indicators from the 2022 Census,
...
#
# Input:
#   data/processed/df_indicators_bradi_2022.csv
#
# Output:
#   data/processed/df_BrADI_2022.csv
#   results/fa_results_2022.csv
#   results/histogram_bradi_2022.png
# =============================================================================
...
df <- read_csv("data/processed/df_indicators_bradi_2022.csv",
...
write_csv(fa_table_2022, "results/fa_results_2022.csv")
...
df$BrADI_2022 <- as.numeric(scale(index_raw)) * 20 + 100

message("  Min: ", round(min(df$BrADI_2022, na.rm = TRUE), 1))
message("  Max: ", round(max(df$BrADI_2022, na.rm = TRUE), 1))
message("  Mean: ", round(mean(df$BrADI_2022, na.rm = TRUE), 1))

# ---- BrADI diagnostics -----------------------------------------------------  

cat("\n=== BrADI summary ===\n")
summary(df$BrADI_2022)
...
hist(df$BrADI_2022,
     main   = "Distribution of BrADI - Year 2022",
     xlab   = "BrADI",
     col    = "steelblue",
     border = "white", 
     xlim = c(60,240), 
...
min_val <- round(min(df$BrADI_2022, na.rm = TRUE), 1)
max_val <- round(max(df$BrADI_2022, na.rm = TRUE), 1)
min_uf <- df$UF[which.min(df$BrADI_2022)]
max_uf <- df$UF[which.max(df$BrADI_2022)]
...
df %>%
  arrange(desc(BrADI_2022)) %>%
  select(UF, Cod_setor, BrADI_2022) %>%
  slice(1:5) %>%
  print()
...
df %>%
  group_by(UF) %>%
  summarise(BrADI_mean = mean(BrADI_2022, na.rm = TRUE)) %>%
  arrange(desc(BrADI_mean)) %>%
  print()

# ---- 4. Save the dataset with BrADI scores -----------------------------------

write_csv(df, "data/processed/df_BrADI_2022.csv")
